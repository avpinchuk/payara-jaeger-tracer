...
public class SenderResolverTest {
    ...
    public void testMultipleImplementationsAmbiguous() throws Exception {
        ...
        Sender sender = getSenderForServiceFileContents("\nio.github.avpinchuk.jaeger.internal.senders.InMemorySenderFactory", true);
    ...
    private Sender getSenderForServiceFileContents(String contents, boolean append) throws Exception {
        String serviceFilePath = "/META-INF/services/io.github.avpinchuk.jaeger.spi.SenderFactory";
        File original = new File(this.getClass().getResource(serviceFilePath).toURI());

        String newContent;
        if (append) {
            try (Scanner s = new Scanner(new FileInputStream(original), Charset.defaultCharset().name())) {
                s.useDelimiter("\\A");
        ...
        Path copy = original.toPath().resolveSibling("io.github.avpinchuk.jaeger.spi.SenderFactory-copy");
